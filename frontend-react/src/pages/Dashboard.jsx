...
import { getSessions, getRemediationStatus, getDriftReport, getPriorityQueue } from '@/lib/api'
...
export default function Dashboard() {
  ...
  const { data: queue }   = useQuery({ queryKey: ['priority_queue'],      queryFn: getPriorityQueue,     refetchInterval: 15_000 })
  const { data: sessions }= useQuery({ queryKey: ['sessions', 20],        queryFn: () => getSessions(20),refetchInterval: 10_000 })
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
        <StatCard label="Total Transcriptions" value={remStat?.total_transcriptions ?? '—'} sub="all sessions" accent={C.cyan} icon={<Mic size={15} />} />
        <StatCard label="Remediation Rate" value={remStat?.remediation_rate != null ? `${remStat.remediation_rate.toFixed(1)}%` : '—'} sub={`${remStat?.remediated ?? 0} remediated`} accent={C.green} icon={<CheckCircle size={15} />} />
        <StatCard label="High-Risk Phonemes" value={highRisk} sub="CUSUM degrading" accent={highRisk > 0 ? C.red : C.textMuted} icon={<TrendingDown size={15} />} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: 14 }}>
        <Card><CardHeader title="Error Distribution" /><CardBody><ErrorTypePie /></CardBody></Card>
        <Card><CardHeader title="Confidence Histogram" subtitle="phoneme-level · 0.5 threshold marked" /><CardBody><ConfidenceHistogram bins={18} /></CardBody></Card>
      </div>
          ...
          <CardBody style={{ padding: 0 }}>
            {(sessions || []).slice(0, 7).map((s, i) => (
              <motion.div key={s.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 20px', borderBottom: i < 6 ? `1px solid ${C.border}` : 'none' }}>
                ...
                {s.cer_score != null && (
                  <span style={{ fontSize: 8, color: s.cer_score < 0.1 ? C.green : C.amber }}>
            ...
            {!sessions?.length && (
              <div style={{ padding: 32, textAlign: 'center', fontSize: 11, color: C.textMuted }}>No sessions yet</div>
            )}
          </CardBody>
        </Card>
          ...
          <CardBody style={{ padding: 0 }}>
            {(drift?.degrading || []).slice(0, 6).map((p, i) => (
              <div key={p.phoneme} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 20px', borderBottom: i < 5 ? `1px solid ${C.border}` : 'none' }}>
                <span style={{ fontWeight: 700, fontSize: 12, color: C.red, minWidth: 32, textAlign: 'right' }}>{p.phoneme}</span>
                <div style={{ flex: 1, height: 4, background: C.border, borderRadius: 2 }}>
                  <div style={{ height: '100%', width: `${(p.avg_confidence * 100).toFixed(0)}%`, background: p.avg_confidence > 0.5 ? C.amber : C.red, borderRadius: 2, transition: 'width 0.5s' }} />
                </div>
                <span style={{ fontSize: 9, color: C.textSecondary, minWidth: 34, textAlign: 'right' }}>{(p.avg_confidence * 100).toFixed(0)}%</span>
                {p.cusum != null && <span style={{ fontSize: 8, color: C.red }}>∑{p.cusum.toFixed(2)}</span>}
              </div>
            ))}
            {!drift?.degrading?.length && (
              <div style={{ padding: 32, textAlign: 'center', fontSize: 11, color: C.green }}>✓ No degrading phonemes</div>
            )}
          </CardBody>
        </Card>
      </div>
      ...
      {lastResult && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <Card glow={C.cyan}>
            <CardHeader title="Last Transcription" subtitle="this session" accent={C.cyan} />
            <CardBody>
              <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: 14, fontSize: 9, color: C.textMuted, flexWrap: 'wrap' }}>
                    <span>Conf: <span style={{ color: C.cyan }}>{(lastResult.confidence * 100).toFixed(1)}%</span></span>
                    ...
                    {lastResult.per_score  != null && <span>PER: <span style={{ color: C.purple }}>{(lastResult.per_score  * 100).toFixed(1)}%</span></span>}
                    {lastResult.snr_db     != null && <span>SNR: <span style={{ color: C.textSecondary }}>{lastResult.snr_db.toFixed(1)} dB</span></span>}
                    <span>Uncertain frames: <span style={{ color: C.textSecondary }}>{lastResult.uncertain_frames}/{lastResult.total_frames}</span></span>
                  </div>
                </div>
